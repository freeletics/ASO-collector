...
class TestExportRatings:
    ...
    def test_get_rating_param_list_return_for_country_and_apps(self):
        PRODUCT_LEN_COUNTRY_APPS = 4
        param_list = export_ratings.get_params_list(
            datetime.datetime(2019, 10, 1), datetime.datetime(2019, 10, 2)
        )
    ...
    def test_get_params_return_platform_and_params(self):
        params = export_ratings.get_params(
    ...
    def test_get_export_data_return_for_all_countries(
        self, rating_three_days_es_request
    ):
        exporter = export.SensorTowerExport()
        param_list = export_ratings.get_params_list(
            datetime.datetime(2019, 10, 1), datetime.datetime(2019, 10, 2)
        )
        exported_data = export_ratings.get_export_data(param_list, exporter)
    ...
    def test_get_row_per_date(self, rating_two_day_two_countries):
        proccessed_data = export_ratings.get_row_per_date(rating_two_day_two_countries)
        assert len(proccessed_data.keys()) == 2

    @mock.patch("exporter.config.COUNTRIES", ["pl", "us"])
    @mock.patch("exporter.config.SENSORTOWER_APPS", {"284882215": "ios"})
    def test_get_row_per_date_has_6_values_per_country(
        self, rating_two_day_two_countries
    ):
        proccessed_data = export_ratings.get_row_per_date(rating_two_day_two_countries)
    ...
    @mock.patch("exporter.config.COUNTRIES", ["pl", "us"])
    @mock.patch("exporter.config.SENSORTOWER_APPS", {"284882215": "ios"})
    def test_get_proccess_data_drop_last_row(self, rating_two_day_two_countries):
        proccessed_data = export_ratings.get_proccessed_data(
    ...
    def test_get_rating_star_cell(self):
        today = {"ES_star_1": 3}
        yesterday = {"ES_star_1": 2}
        rating_start_value = export_ratings.get_rating_star_cell(
            "ES", 1, today, yesterday
        )
        assert rating_start_value[1] == 1

    def test_get_export_field_list(self):
        field_list = export_ratings.get_export_field_list(["pl"])
